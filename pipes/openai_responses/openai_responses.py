...
version: 0.9.9
license: MIT
commit: 070bf9bdafa5fb2c64f46c237154da7d9db5d2a6
"""
...
class APIError(Exception):
    def __init__(self, response: aiohttp.ClientResponse, error_data: dict):
        base_error_msg = f"API Error ({response.status} {response.reason})"
        try:
            message = error_data.get("error", {}).get("message")
            if message:
                super().__init__(message)
            else:
                raw_text = str(error_data)
                super().__init__(
                    f"{base_error_msg}: Unexpected error structure. Response: {raw_text[:200]}"
                )
        except Exception as e:
            super().__init__(
                f"{base_error_msg}: An error occurred while parsing the error response: {e}"
            )
...
class Pipe:
    ...
    def get_model_name(self, model_id: str) -> str:
        ...
        # 1. Strip trailing version/date suffixes for a cleaner base ID.
        # Handles formats like -002, -2024-05-13, -preview, etc.
        base_id = re.sub(
            r"-(?:\d{3,}|\d{4}-\d{2}-\d{2}|latest|preview|stable)$", "", model_id
        )

        parts = base_id.split("-")
        processed_parts = []

        # 2. Process parts in a single pass.
        i = 0
        while i < len(parts):
            part = parts[i]

            # Special case: Combine "GPT" or "ChatGPT" with a version number.
            # This avoids the "split-then-fix" logic of the original.
            if (
                part.lower() in ("gpt", "chatgpt")
                and (i + 1) < len(parts)
                and parts[i + 1][:1].isdigit()
            ):
                version = parts[i + 1]
                formatted_name = f"{MODEL_NAME_MAP[part.lower()]}-{version}"
                processed_parts.append(formatted_name)
                i += 2  # Skip the next part (the version number)
                continue

            # General case: Look up in the map or capitalize.
            # .get() with a default value is cleaner than multiple .replace() calls.
            formatted_part = MODEL_NAME_MAP.get(part.lower(), part.capitalize())
            processed_parts.append(formatted_part)
            i += 1

        # 3. Join the processed parts into the final string.
    ...
    async def pipes(self):
        ...
        return [
            {"id": model_id, "name": self.get_model_name(model_id)}
    ...
    async def pipe(
        ...
        self.logger.info("Model ID: %s", openwebui_model_id)
        ...
        async with self.session.post(url, json=request_body, headers=headers) as resp:
            if not resp.ok:
                try:
                    ...
                    resp_data = None
                self.logger.info(f"Error on {url} with {request_body} -> {resp_data}")
                raise APIError(resp, resp_data)
        ...
        async with self.session.post(url, json=request_params, headers=headers) as resp:
            if not resp.ok:
                try:
                    ...
                    resp_data = None
                self.logger.info(f"Error on {url} with {request_params} -> {resp_data}")
                raise APIError(resp, resp_data)
