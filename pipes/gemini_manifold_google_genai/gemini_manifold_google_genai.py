...
class GeminiContentBuilder:
    ...
    def _fetch_and_validate_chat_history(
        ...
        if chat := Chats.get_chat_by_id_and_user_id(
            ...
            log.trace(
                "Fetched messages from database:", payload=chat_content.get("messages")
            )
...
class Pipe:
    ...
    def _get_or_create_genai_client(
        ...
        try:
            ...
            raise GenaiApiError(f"{api} Genai client initialization failed: {Pipe._format_error(e)}") from e
    ...
    async def _get_genai_models(
        ...
        fetch_both = bool(
            use_vertex_ai and vertex_project and (free_api_key or paid_api_key)
        )
    ...
    async def _fetch_models_from_client_internal(
        ...
        try:
            ...
            log.error(f"Retrieving models from {source_name} failed: {self._format_error(e)}")
    ...
    def _build_gen_content_config(
        ...
        if enable_url_context:
            # Check capability from open_webui.config
            is_compatible = False
            if model_id in config:
                is_compatible = (
                    config[model_id].get("capabilities", {}).get("url_context", False)
                )
        ...
        if (
            __tools__ is not None
            and __metadata__.get("params", {}).get("function_calling") == "native"
        ):
            for name, tool_def in __tools__.items():
                if not name.startswith("_"):
                    ...
                    gen_content_conf.tools.append(tool)
    ...
    async def _unified_response_processor(
        ...
        try:
            ...
            error_occurred = True
            error_msg = f"Response processing ended with error: {self._format_error(e)}"
            log.exception(error_msg)
            await event_emitter.emit_error(error_msg)
            ...
            if not error_occurred and response_parts:
                ...
                log.trace(
                    "Stored response parts in state for Filter.outlet.",
                    payload=response_parts,
                )
    ...
    def _get_toggleable_feature_status(
        ...
        # HACK
        if filter_id in ("gemini_paid_api", "gemini_vertex_ai_toggle"):
            return (False, False)
        if filter_id == "gemini_reasoning_toggle":
            features = __metadata__.get("features")
            if "reason" in features:
                log.info(
                    f"Feature '{filter_id}' is available and toggled by custom toggle"
                )
